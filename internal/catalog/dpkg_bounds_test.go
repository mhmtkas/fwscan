...
func TestParseStatusBounds(t *testing.T) {
	t.Run("too many stanzas", func(t *testing.T) {
		...
		if _, err := parseStatus(strings.NewReader(b.String()), OSRelease{}); err == nil {
	...
	t.Run("single line longer than the scanner buffer", func(t *testing.T) {
		...
		if _, err := parseStatus(strings.NewReader(status), OSRelease{ID: "debian", Codename: "", VersionID: ""}); err == nil {
	...
	t.Run("continuation before any field is ignored", func(t *testing.T) {
		...
		got, err := parseStatus(strings.NewReader(status), OSRelease{ID: "debian", Codename: "", VersionID: ""})
		if err != nil {
